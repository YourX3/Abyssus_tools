...
function init(){
	...
	textVersion.innerHTML = '<font size="1" color="white">Abyssus Tools V 0.5 __ Last Updtate 08/06/2018 17h47 </font>';
	...
	if(readCookie("exploTotal") === null){
		...
		if(Number(readCookie("exploCurrent")) < Number(readCookie("exploTotal"))){
			...
			if(Number(readCookie("exploCurrent"))+1 === Number(readCookie("exploTotal"))){}
				...
				for(var i=1; i < 14; ++i){
					getElementsByTagNameInList(getElementsByTagNameInList(trList[i].childNodes, "TD")[4].childNodes, "INPUT")[0].value = readCookie("explo_" + String(i+1));
				}
			}
