	...
	if(sessionStorage.getItem("displayingDistances_total") === null){
		...
		$.post('ajax/ennemies.php', {mintdc:playerTM, maxtdc:maxTM, page:1, tri:'distance', sens:'asc', guerre:0, paix:0, ally:0}, function(data){
			...
			if(listOfResults === null){
				document.getElementsByTagName('tbody')[1].childNodes[3].childNodes[3].textContent += "\r" + " ___________________ Distance et temps de Trajet inconnus..";
			}
			else{
				var distance = listOfResults[3];
				var time = listOfResults[4];
	
				document.getElementsByTagName('tbody')[1].childNodes[3].childNodes[3].textContent += "\r" + " ___________________ Distance: " + distance + " _Temps de trajet: " + time;
			}
		});
	}
	else{
